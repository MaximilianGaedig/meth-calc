...
pub enum Args {
	#[clap(name = "solvex")]
	SolveForX {
		/// The 2 sided equation to solve for x
		#[clap(name = "equation")]
		equation: String,
	},
	#[clap(name = "shunting")]
	Shunting {
		/// The 2 sided equation to solve using the shunting yard algorithm
		#[clap(name = "equation")]
		equation: String,
	},
	#[clap(name = "spoly")]
...
pub async fn main(args: Args) -> Result<ReturnCode> {
	...
	println!(
		"{0}",
		match args {
			Args::Shunting { equation } => shunting_yard::solve(equation),
			_ => Ok("Not implemented yet".to_string()),
		}?
	);
	ctrlc.recv().await;
